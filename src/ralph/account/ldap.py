...
try:
    ...
    class MappedNestedGroupOfNamesType(NestedGroupOfNamesType):
        ...
        def user_groups(self, ldap_user, group_search):
            # filter groups to these presents in config if mapping defined
        ...
        def group_name_from_info(self, group_info):
            # map ldap group names into ralph group names if mapping defined
            name = super(MappedNestedGroupOfNamesType,
                         self).group_name_from_info(group_info)
            if self._ldap_groups:
                mapped = self._ldap_groups.get(name)
            return mapped
