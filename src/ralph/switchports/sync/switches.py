...
def iter_rack_switches(rack_configuration: RackConfiguration) -> list[DataCenterAsset]:
    ...
    switch_configs = (
        rack_configuration.switches.filter(backend_validation=True)
        .select_related("switch")
        .prefetch_related("overrides__switch")
    )
...
def disabled_only_switch_ids(rack_configuration: RackConfiguration) -> set[int]:
    ...
    for sc in (
        rack_configuration.switches.filter(backend_validation=False)
        .select_related("switch")
