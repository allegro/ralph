...
def extract_asset(remote_asset: RemoteAsset) -> tuple[MaybeDCA, MaybeDCA, MaybeDCA]:
    """
    Try to extract a DataCenterAsset from the given RemoteAsset
    ...
    3. Match remote_mac_from_remote_id to an Ethernet MAC address and
      return the associated DataCenterAsset

    Returns a tuple of (DataCenterAsset or None, DataCenterAsset or None, DataCenterAsset or None)
    where each element corresponds to the result of each method.
    """
...
def cross_validate(*assets: MaybeDCA) -> bool | None:
    ...
    Returns:
     - True if all non-None assets are the same,
     - False if there are conflicting non-None assets,
     - None if all assets are None.
    """
