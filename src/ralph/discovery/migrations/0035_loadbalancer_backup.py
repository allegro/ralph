...
class Migration(DataMigration):
    ...
    tables = [
        'discovery_loadbalancervirtualserver',
        'discovery_loadbalancermember',
        'discovery_loadbalancerpool',
    ]

    def forwards(self, orm):
        print(
            "WARNING! In this migration load balancers tables will be "
            "truncated. All data will be copied to tables with _backup suffix."
        )
        # backup data
        for table in self.tables:
            backup_table = table + '_backup'
            try:
                db.execute("CREATE TABLE {} LIKE {}".format(backup_table, table))
                db.execute("INSERT INTO {} SELECT * FROM {}".format(backup_table, table))
            except DatabaseError:
                print("Table {} already exist".format(backup_table))
            else:
                print("All data from {} copied to {}".format(table, backup_table))

        # clear original tables
        for table in self.tables:
            db.execute('DELETE FROM {}'.format(table))
            print("Table {} cleared".format(table))
