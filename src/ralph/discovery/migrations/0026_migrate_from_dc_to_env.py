...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        cache = {}
        for data_center in orm['discovery.DataCenter'].objects.all():
            env = orm['discovery.Environment'].objects.create(
                data_center=data_center,
                name=data_center.name,
                hosts_naming_template=data_center.hosts_naming_template,
                next_server=data_center.next_server,
                domain=data_center.domain,
            )
            cache[data_center.pk] = env
        for network in orm['discovery.Network'].objects.filter(
            data_center__isnull=False,
        ):
            network.environment = cache.get(network.data_center.pk, None)
            network.save()
