...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        devices = orm['discovery.OperatingSystem'].objects \
            .values('device') \
            .annotate(devices_num=models.Count('device')) \
            .filter(devices_num__gt=1) \
            .order_by('device')
        for item in devices:
            operating_systems = orm['discovery.OperatingSystem'].objects \
                .filter(device=item['device'])
            to_delete = [os.pk for os in operating_systems[1:]]
            orm['discovery.OperatingSystem'].objects \
                .filter(pk__in=to_delete) \
                .delete()
