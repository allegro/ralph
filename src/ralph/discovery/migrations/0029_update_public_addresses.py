...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ip in orm['discovery.IPAddress'].objects.all():
            try:
                ip_addr = ipaddr.IPAddress(ip.address)
                ip.is_public = not ip_addr.is_private
                ip.save()
            except ValueError:
                print ('Bad IP Address {0}'.format(ip.address))
