...
class TestSearchForm(TestCase):
    ...
    def test_model_field(self):
        ...
        self.assertEqual(len(get_rows_from_table), 2)

        # Test if search form find correct data
        self.assertItemsEqual(
            [asset.model.name for asset in get_rows_from_table],
            ['Model1', 'Model1']
        )
        self.assertItemsEqual(
            [asset.sn for asset in get_rows_from_table],
            ['1234-1234-1234-1234', '1235-1235-1235-1235']
        )

        # What do Ralph when we don't insert model name? (return all asset)
        get_content = self.client.get('/assets/dc/search?model=')
        self.assertEqual(get_content.status_code, 200)
        empty_model_rows = get_content.context_data['page'].object_list
        self.assertEqual(len(empty_model_rows), 3)

        # or we insert wrong model name (outside range)?
        get_content = self.client.get('/assets/dc/search?model=Ralph0')
        self.assertEqual(get_content.status_code, 200)
        outside_range_rows = get_content.context_data['page'].object_list
    ...
    def test_invoice_no_field(self):
        ...
        # Test if search form find correct data
        self.assertItemsEqual(
            [asset.invoice_no for asset in get_rows_from_table],
    ...
    def test_order_no_field(self):
        ...
        # Test if search form find correct data
        self.assertItemsEqual(
            [asset.order_no for asset in get_rows_from_table],
            ['Order No1', 'Order No1']
        )
        self.assertItemsEqual(
            [asset.sn for asset in get_rows_from_table],
    ...
    def test_provider_field(self):
        ...
        get_rows_from_table = get_content.context_data['page'].object_list
        self.assertEqual(len(get_rows_from_table), 1)

        # Test if search form find correct data
        self.assertEqual(get_rows_from_table[0].provider, 'Provider2')
        self.assertEqual(get_rows_from_table[0].sn, '1235-1235-1235-1235')
    ...
    def test_status_field(self):
        ...
        # Test if search form find correct data
        self.assertItemsEqual(
            [AssetStatus.name_from_id(asset.status) for asset in get_rows_from_table],
            ['used']
        )
        self.assertEqual(get_rows_from_table[0].sn, '1236-1236-1236-1236')
    ...
    def test_sn_field(self):
        ...
        # Test if search form find correct data
        self.assertEqual(get_rows_from_table[0].sn, '1234-1234-1234-1234')
...
class TestSearchDataRangeFields(TestCase):
    ...
    def test_start_date_is_less_then_end_date(self):
        ...
        get_rows_from_table = get_content.context_data['page'].object_list
        self.assertEqual(len(get_rows_from_table), 0)
