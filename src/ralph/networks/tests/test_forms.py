...
class NetworkInlineWithDHCPExposeTestCase(RalphTestCase):
    ...
    def test_dhcp_expose_for_new_record_duplicate_hostname_should_not_pass(
        ...
        self.assertEqual(response.status_code, 200)
        msg = (
            'Hostname "{hostname}" is already exposed in DHCP in {dc}.'.format(  # noqa
    ...
    def test_dhcp_expose_without_address_for_existing_record_should_not_pass(
        self,
    ):  # noqa
        inline_data = {
            "TOTAL_FORMS": 1,
    ...
    def test_dhcp_expose_without_hostname_for_existing_record_should_not_pass(
        self,
    ):  # noqa
        inline_data = {
            "TOTAL_FORMS": 1,
