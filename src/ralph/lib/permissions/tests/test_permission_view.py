...
class PermissionsByFieldTestCase(TestCase):
    ...
    def test_codename(self):
        self.assertEqual(SimplePermissionView.permision_codename, self.codename)

    def test_user_not_perm(self):
        self.client.login(username=self.user_not_perm.username, password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 403)

    def test_user_with_perm(self):
        self.client.login(username=self.user_with_perm.username, password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)

    def test_user_root(self):
        self.client.login(username=self.root.username, password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
