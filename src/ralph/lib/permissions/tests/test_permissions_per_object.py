...
class TestUserPermissions(TestCase):
    ...
    def test_is_author_and_collaborator(self):
        ...
        self.assertEqual(
            str(result),
            "(AND: ('author', {u}), ('collaborators', {u}))".format(u=repr(self.user1)),
        )
    ...
    def test_is_author_or_collaborator(self):
        ...
        self.assertEqual(
            str(result),
            "(OR: ('author', {u}), ('collaborators', {u}))".format(u=repr(self.user1)),
        )
    ...
    def test_is_author_or_collaborator_when_superuser_and_skip_superuser_rights(
        self,
    ):  # noqa
...
class TestPermissionsPerObject(PermissionsTestMixin, TestCase):
    ...
    def test_author_should_have_access_to_his_article(self):
        self.assertTrue(self.article_2.has_permission_to_object(self.user2))

    def test_superuser_should_have_access_to_any_article(self):
        self.assertTrue(self.article_1.has_permission_to_object(self.superuser))
    ...
    def test_user_should_not_have_access_to_not_his_artictle(self):
        ...
        self.assertFalse(self.article_2.has_permission_to_object(self.user3))
        # article with collaborators
        self.assertFalse(self.article_1.has_permission_to_object(self.user3))

    def test_superuser_should_have_access_to_any_long_article(self):
        self.assertTrue(self.long_article.has_permission_to_object(self.superuser))

    def test_author_should_have_access_to_long_article_when_it_is_long(self):
        self.assertTrue(self.long_article.has_permission_to_object(self.user1))

    def test_author_should_not_have_access_to_long_article_when_it_is_not_long(
        self,
    ):  # noqa
        self.assertFalse(self.long_article_2.has_permission_to_object(self.user1))
...
class TestPermissionsPerObjectForm(PermissionsTestMixin, TestCase):
    ...
    def test_save_valid_foreign_key_should_pass(self):
        form = self.SampleForm(
            {"lead_article": self.article_1.id, "articles": [self.long_article.id]},
            _user=self.user1,
        )
    ...
    def test_save_invalid_foreign_key_should_have_form_errors(self):
        form = self.SampleForm(
            {"lead_article": self.article_2.id, "articles": [self.long_article.id]},
            _user=self.user1,
        )
    ...
    def test_save_invalid_m2m_should_have_form_errors(self):
        form = self.SampleForm(
            {"lead_article": self.article_1.id, "articles": [self.long_article_2.id]},
            _user=self.user2,
        )
        self.assertFalse(form.is_valid())
        self.assertEqual(
