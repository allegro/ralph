...
EXTERNAL_JOBS_METRIC_PREFIX = getattr(settings, "EXTERNAL_JOBS_METRIC_PREFIX", "jobs")
...
JOB_NOT_ENDED_STATUSES = set([JobStatus.QUEUED, JobStatus.STARTED, JobStatus.FROZEN])
...
class Job(TimeStampMixin):
    ...
    def user(self):
        try:
            ...
            if self.username:
                try:
                    return get_user_model()._default_manager.get(username=self.username)
    ...
    def _update_dumped_params(self):
        ...
        logger.debug("Updating _dumped_params to {}".format(self._dumped_params))
        self.save()
    ...
    def prepare_params(cls, **kwargs):
        ...
        result["_request__user"] = result.get("_request__user") or (
    ...
    def dump_obj_to_jsonable(cls, obj):
        ...
        if isinstance(obj, (list, tuple, set)):
            ...
            result = {
                ...
                "content_type_id": ContentType.objects.get_for_model(obj.model).pk,
            }
        elif isinstance(obj, date):
            result = {"__date": True, "value": str(obj)}
    ...
    def _restore_django_models(cls, obj):
        ...
        if isinstance(obj, (list, tuple)):
            ...
            if obj.get("__date") is True:
                ...
                result = ct.model_class().objects.filter(pk__in=obj.get("value"))
