...
class RalphIronicClient:
    ...
    def get_nodes_list(self, associated=True, fields=None):
        ...
        return self.ironic_client.node.list(associated=associated, fields=fields)
...
class RalphOpenstackClient:
    ...
    def get_flavors_list(self):
        ...
        if len(flavors) == 0:
            raise EmptyListError(
                "Got an empty list of flavors from {}".format(self.site["auth_url"])
            )
        return flavors
    ...
    def get_servers_list(self, search_opts=None):
        ...
        while True:
            try:
                logger.debug("Fetching servers with marker {}".format(marker))
        ...
        if len(servers) == 0 and not search_opts.get("changes-since"):
            raise EmptyListError(
                "Got an empty list of instances from {}".format(self.site["auth_url"])
            )
        logger.info("Fetched {} servers from {}".format(len(servers), self.site["tag"]))
        return servers
...
class RalphOpenStackInfrastructureClient:
    ...
    def _get_instances_from_settings(self):
        ...
        for os_instance in settings.OPENSTACK_INSTANCES:
            if (
                os_instance.get("provider", DEFAULT_OPENSTACK_PROVIDER_NAME)
    ...
    def get_openstack_projects(self):
        ...
        for client in self.clients:
            ...
            for project in client.get_keystone_projects():
                ...
                openstack_projects[project.id]["tags"].append(client.site["tag"])
        ...
        for client in self.clients:
            ...
            for server in client.get_servers_list(search_opts=search_opts):
                ...
                for zone in server["addresses"]:
                    if "network_regex" in client.site and not re.match(
                        client.site["network_regex"], zone
                    ):
                        continue
                ...
                # from DNS
                new_server["hostname"] = new_server["hostname"] or server["name"]
                try:
                ...
                if flavor_id not in openstack_flavors:
                    logger.warning(
                        "Flavor %s (found in host %s) not in flavors list. Fetching it",
                        flavor_id,
                        host_id,
                    )
