...
class CustomFieldsAPITests(APITestCase):
    ...
    def test_get_configuration_variables_in_object_resource(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["configuration_variables"],
            {
                "test_choice": "qwerty",
            },
        )
    ...
    def test_get_customfields_for_single_object(self):
        ...
        self.assertEqual(cfv["custom_field"]["name"], self.custom_field_str.name)
    ...
    def test_delete_custom_field_value_with_unmatching_managing_group_should_fail(
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(CustomFieldValue.objects.filter(pk=self.cfv1.pk).count(), 1)
    ...
    def test_delete_custom_field_value_with_matching_managing_group_should_pass(
        ...
        self.assertEqual(CustomFieldValue.objects.filter(pk=self.cfv1.pk).count(), 0)

    def test_add_new_customfield_value_should_send_api_post_create_signal(
        self,
    ):  # noqa: E501
        ...
        self.assertEqual(self._sig_called_with_instance.id, response.data["id"])
    ...
    def test_add_new_customfield_value_with_duplicated_customfield_should_not_pass(
        self,
    ):  # noqa
        url = reverse(self.list_view_name, args=(self.sm1.id,))
        data = {
    ...
    def test_update_customfield_value_with_duplicated_customfield_should_not_pass(
        self,
    ):  # noqa
        url = reverse(
            self.detail_view_name,
