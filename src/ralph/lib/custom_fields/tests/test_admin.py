...
class CustomFieldValueAdminMaxinTestCase(TestCase):
    ...
    def test_get_customfield_formfield_for_choicefield(self):
        response = self.client.get(
            reverse("admin:customfield_formfield", args=(self.custom_field_choices.id,))
        )
        self.assertContains(response, '<select name="__empty__">')
        # default_value should be selected
        self.assertContains(response, '<option value="zxcvbn" selected>zxcvbn</option>')
    ...
    def test_custom_field_not_in_form_for_nonmatching_managing_group(self):
        ...
        filled_in_custom_field_forms = [
            form
            for form in response.context_data["inline_admin_formsets"][
                0
            ].formset.forms  # noqa: E501
    ...
    def test_edit_custom_field_value_fails_with_nonmatching_managing_group(
        self,
    ):  # noqa: E501
        group = GroupFactory()
