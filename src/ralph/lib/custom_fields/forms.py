...
class CustomFieldValueForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if custom_field:
            if (
                custom_field.managing_group is not None
                and custom_field.managing_group not in self.request.user.groups.all()  # noqa: E501
            ):
                self.add_error(
                    "custom_field",
                    ValidationError(
                        _("Only users from {} group can set this custom field.").format(
                            custom_field.managing_group.name
                        ),
                        code="invalid",
                    ),
                )
