...
class TestChecks(TransitionTestCase):
    ...
    def test_transition_templates_item_should_be_two_elements_tuple(self):
        errors = check_transition_templates((("broken",), 1234, "foo-bar.html"))
        expected_errors = [
            Error("Element #0 must be a two elements tuple", id="transitions.E003"),
            Error("Element #1 must be a two elements tuple", id="transitions.E003"),
            Error("Element #2 must be a two elements tuple", id="transitions.E003"),
        ]
        self.assertEqual(expected_errors, errors)
