...
class TransitionActionTestMixin(object):
    def setUp(self):
        ...
        self.bo = BackOfficeAssetFactory(
            status=BackOfficeAssetStatus.new, user=UserFactory(), owner=UserFactory()
        )
    ...
    def prepare_gui_data(self):
        return {
            ...
            "assign_office_infrastructure__office_infrastructure": self.officeinfrastructure.id,
...
class TransitionActionTest(TransitionActionTestMixin, TransitionTestCase):
    def test_sync_api(self):
        response = self.api_client.post(
            reverse("transitions-view", args=(self.transition_1.id, self.bo.pk)),
    ...
    def test_sync_api_validation_error(self):
        ...
        data["user"] = ""
        response = self.api_client.post(
            reverse("transitions-view", args=(self.transition_1.id, self.bo.pk)), data
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["user"], ["This field may not be blank."])
    ...
    def test_sync_gui(self):
        ...
        self.assertTrue(
            ...
            reverse("admin:back_office_backofficeasset_change", args=(self.bo.id,)),
        )
        bo = BackOfficeAsset.objects.get(pk=self.bo)
        self.assertEqual(bo.owner_id, self.user.id)
        self.assertEqual(bo.warehouse_id, self.warehouse_1.id)
    ...
    def test_api_options(self):
        request = self.api_client.options(
            reverse("transition-view", args=(self.transition_1.id, self.bo.pk))
        )
...
class TestAsyncActions(
    ...
    def test_async_gui_running_new_when_another_in_progress_should_return_error(
        self,
    ):  # noqa
    ...
    def test_async_api_running_new_when_another_in_progress_should_return_error(
        self,
    ):  # noqa
        # mock another async job running
        TransitionJob.objects.create(
        ...
        response = self.api_client.post(
            reverse("transition-view", args=(self.transition_2.id, self.bo.pk)),
            self.prepare_api_data(),
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
