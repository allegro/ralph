...
class BaseObjectForeignKeyTestCase(TestCase):
    def test_limit_choices(self):
        model = BaseObjectForeignKeyModel.objects.create(base_object=BaseObject.objects.create())
        bo_field = model._meta.get_field("base_object")
        content_types = ContentType.objects.get_for_models(BackOfficeAsset, DataCenterAsset)
