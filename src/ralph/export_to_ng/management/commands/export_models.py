...
import ralph_assets
APP_MODELS = {model._meta.object_name: model for model in get_models(app_mod='ralph_assets')}
APP_MODELS.update({
...
def export_model(model_name, export_filepath):
    '''
    Exports models:
        model_name: model from RalphNG eg. AssetModel, DataCenterAsset
        export_filepath: file path where data should be exported
    '''
    model_resource = get_resource(model_name)
    ...
    with open(export_filepath, 'wb') as output:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        export_filename = '.'.join([options['model_name'], 'csv'])
        export_path = os.path.join(options['export_dir'], export_filename)
        export_model(options['model_name'], export_path)
