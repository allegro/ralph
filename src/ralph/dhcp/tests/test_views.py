...
class DHCPConfigTest(TestCase):
    def test_config_endpoint_should_return_200(self):
        ...
        url = "{}?env={}".format(reverse("dhcp_config_entries"), network.network_environment)
...
class DHCPEntriesViewTest(TestCase):
    ...
    def test_get_last_modified_should_return_ip_modified(self):
        ...
        returned = self.view.get_last_modified(Network.objects.filter(id__in=[network.id]))
        self.assertEqual(
    ...
    def test_filter_duplicated_hostnames(self):
        ...
        entries = self.view._get_dhcp_entries(Network.objects.filter(id__in=[network.id]))
