...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Bounds all CIOwners to Profiles based on username. If profile is
        not found - the CIOwner is deleted"""
        for owner in orm['cmdb.CIOwner'].objects.all():
            try:
                owner.profile = orm['account.Profile'].objects.get(
                    user__username=owner.sAMAccountName
                )
                owner.save()
            except orm['account.Profile'].DoesNotExist:
                print("User {} doesn't exist. Deleting owner".format(
    ...
    def backwards(self, orm):
        """Recover the legacy CIOwner data from the profile"""
        for owner in orm['cmdb.CIOwner'].objects.all():
            if not owner.profile:
                continue
            owner.sAMAccountName = owner.profile.user.username
            owner.first_name = owner.profile.user.first_name
            owner.last_name = owner.profile.user.last_name
            owner.email = owner.profile.user.email
            owner.save()
    ...
    models = {
        ...
        'cmdb.ciowner': {
            ...
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'profile': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['account.Profile']", 'unique': 'True', 'null': 'True'}),
