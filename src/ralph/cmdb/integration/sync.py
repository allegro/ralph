...
def integrate_main():
    """
    From now, collect all plugins, add to the commandline, and
    run if required. May be multiple specified options.
    """
    required_plugin_name = re.compile('[^-]+')
    usage = "usage: %prog "

    # Populate usage cmdline
    cmdb_plugins = plugin.BY_NAME['cmdb'].iteritems()
    for p in cmdb_plugins :
        plugin_name = p[0]
        usage = "%s--%s " % (usage, plugin_name)
    parser = OptionParser(usage)

    # Populate OptionParser with options
    cmdb_plugins = plugin.BY_NAME['cmdb'].iteritems()
    for p in cmdb_plugins:
        plugin_name = p[0]
        ...
        parser.add_option('--%s' % plugin_name,
                dest="%s" % plugin_name,
                action="store_true",
    ...
    context=dict(parser=parser)
    specified_option = False
    (options, args) = parser.parse_args()
    # Run enabled plugins in CMDB chain
    cmdb_plugins = plugin.BY_NAME['cmdb'].iteritems()
    for p in cmdb_plugins:
        if getattr(options,p[0]):
            func_name = p[1]
